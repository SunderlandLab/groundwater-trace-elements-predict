...
calc.TRI.impact <- function(xrow, df, SEMS = FALSE) { 
  ...
  if (nrow(ptsrc)>0) {
    for (j in 1:nrow(ptsrc)) {
      ...
      dist = distm(st_coordinates(xrow), st_coordinates(ptsrc[j,]), fun=distHaversine)/1000
...
extract_TRI_impact <- function(M_stn, metal.code) {
  ...
  if(!file.exists(paste0('CoVar/TRI/TRI.', metal.code,'.onsite.cumulativeSums.csv'))){
    ...
    TRI.onsite <- read.csv(paste0('CoVar/TRI/TRI.', metal.code,'.onsite.cumulativeSums.csv')) %>%
    ...
    if (nrow(wells.list)>0) {
      wells.list <- wells.list %>%
        ...
        lapply(function(x) calc.TRI.impact(x, TRI.onsite)) %>%
        # convert back to a data frame
        bind_rows()
      
    # Add impacts as a column to M_stn
    M_stn <- M_stn %>%
      left_join(wells.list %>% st_drop_geometry() %>% dplyr::select("location.id", TRI.impact), by = "location.id") %>%
    ...
    # A) prepare site data
    # read in dataframe 
    TRI.water <- read.csv(paste0('CoVar/TRI/TRI.', metal.code,'.water.cumulativeSums.csv')) %>% 
      st_as_sf(coords = c("LONGITUDE", "LATITUDE"),
               crs = 4326) %>%
      prepare_well_impact()
    ...
    if (nrow(wells.list)>0) {
      wells.list <- wells.list%>%
        ...
        group_split() %>% # Split rows into a list
        # apply calc.TRI.impact to each element in wells.list
        lapply(function(x) calc.TRI.impact(x, TRI.water)) %>%
        # convert back to a data frame
        bind_rows()  
...
extract_SEMS_impact <- function(M_stn, metal.code) {
  ...
  if(nrow(wells.list)>0){
    wells.list<- wells.list%>%
      ...
      group_split() %>% # Split rows into a list
      # apply calc.TRI.impact to each element in wells.list
      lapply(function(x) calc.TRI.impact(x, SEMS, SEMS = TRUE)) %>%
      # convert back to a data frame
      bind_rows()  
