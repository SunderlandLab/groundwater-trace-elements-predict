...
predictor_extract_function <- function(metal.code) {
  ...
  M_stn <- st_as_sf(M_stn,
                    coords = c("longitude", "latitude"),
                    crs = st_crs(mapUSm))# setting initial crs
  
  ##  Step 2. Environmental predictors: extract values to wells--------------------------------------------------------------------------
  # 1) Precipitation (mm)
  M_stn <- extract_raster_values(raster_path = 'CoVar/PRISM_ppt_30yr_normal_800mM2_annual_asc/PRISM_ppt_30yr_normal_800mM2_annual_asc.asc',
                                 points_sf = M_stn,
                                 column_name = "ppt")
  
  # 2) Annual Mean Temp (F)
  M_stn <- extract_raster_values(raster_path = 'CoVar/PRISM_tmean_30yr_normal_800mM2_annual_asc/PRISM_tmean_30yr_normal_800mM2_annual_asc.asc',
                                 points_sf = M_stn,
                                 column_name = "tmean")
  
  # 3) Groundwater flow parameters from Reitz et al. 2017 (800 m resolution)
  M_stn <- extract_raster_values(raster_path = 'CoVar/reitz_hydrology/QuickFlow_0013/RO_0013.tif',
                                 points_sf = M_stn,
                                 column_name = "quickflow")
  
  # 4) Recharge
  M_stn <- extract_raster_values(raster_path = 'CoVar/reitz_hydrology/EffRecharge_0013_v2/0013/RC_eff_0013.tif',
                                 points_sf = M_stn,
                                 column_name = "effrech")
  
  # 5) ET
  M_stn <- extract_raster_values(raster_path = 'CoVar/reitz_hydrology/ET_0013/ET_0013.tif',
                                 points_sf = M_stn,
                                 column_name = "ET")
  
  # 3b) Original recharge from Wolock et al. for comparison
  M_stn <- extract_raster_values(raster_path = 'CoVar/rech48grd/rech48grd/w001001x.adf',
                                 points_sf = M_stn,
                                 column_name = "rech")
  
  # 4) Soil Organic Content, (tons/Hectare)
  M_stn <- extract_raster_values(raster_path = 'CoVar/HWSDa_OC_Dens_30SEC_US.asc',
                                 points_sf = M_stn,
                                 column_name = "soiloc")
  ...
  geochem <- read.table(
    'CoVar/soil_geochemistry/Appendix_4b_Chorizon_18Sept2013.txt',
    header = T,
    # first row is unit
    sep = '\t',
    stringsAsFactors = F
  ...
  fx = function(chemName) {
    ...
    r <- raster(extent(geochemOut),
                res = 0.5,
                crs = proj4string(geochemOut))
  ...
  M_stn <- st_join(M_stn, hlrus[c("SLOPE",
                                  "SAND",
                                  "RELIEF",
                                  "PFLATTOT",
                                  "PFLATLOW",
  ...
  M_stn <- extract_vector_values(
    M_stn,
    "CoVar/Soller_surfgeo/USGS_DS_425_SHAPES",
    "Surficial_materials",
    "UNIT_NAME"
  ) |>
  ...
  M_stn <- extract_raster_values(
    'CoVar/usgs_tiledrainage/SubsurfaceDrainExtentUS_90s_agg.tif',
    M_stn,
    'drainage',
    method = 'bilinear'
  )
  ...
  # 13) stream density
  M_stn <- extract_raster_values('CoVar/stmdenhuc12/w001001.adf', M_stn, 'stn_den', method = 'bilinear')
  
  # 14) Surficial groundwater flow parameters (MODFLOW 6) (250 m resolution)
  M_stn <- extract_raster_values(
    'CoVar/modflow6_surfgw/Output_CONUS_trans_dtw/conus_MF6_SS_Unconfined_250_dtw.tif',
    M_stn,
    'dtw',
    buffer_on = TRUE
  )
  M_stn <- extract_raster_values(
    'CoVar/modflow6_surfgw/Output_CONUS_trans_dtw/conus_MF6_SS_Unconfined_250_trans.tif',
    M_stn,
    'trans',
    buffer_on = TRUE
  )
  M_stn <- extract_raster_values(
    'CoVar/modflow6_surfgw/Output_CONUS_unsat_traveltime/conus_MF6_SS_Unconfined_250_tt_total.tif',
    M_stn,
    'unsatTT',
    buffer_on = TRUE
  )
  M_stn <- extract_raster_values(
    'CoVar/modflow6_surfgw/Output_CONUS_unsat_watercontent/conus_MF6_SS_Unconfined_250_wc_avg.tif',
    M_stn,
    'unsatWC',
    buffer_on = TRUE
  )
  ...
  for (filename in list.files(path = "CoVar/soil_properties")) {
    if (grepl("int", filename)) {
      M_stn <- extract_raster_values(
        paste0("CoVar/soil_properties/", filename),
        M_stn,
  ...
  for (filename in list.files(path = "CoVar/SSURGO90m/well_data")) {
    ...
    tbl = tbl %>% dplyr::select('location_id', 'MEAN') %>% dplyr::rename(!!name := 'MEAN', 'location.id' =
                                                                           'location_id')
  ...
  M_stn <- extract_raster_values(raster_path = 'CoVar/USGS_NO3/no3_doms.asc',
                                 points_sf = M_stn,
                                 column_name = "no3_dom")
  # groundwater no3 predicted at public well depths
  M_stn <- extract_raster_values(raster_path = 'CoVar/USGS_NO3/no3_pubs.asc',
                                 points_sf = M_stn,
                                 column_name = "no3_pub")
  ...
  saveRDS(M_stn,
          paste0("R_Output/", metal.code, "_RawPredictors.rds"))
